<!doctype html>
<html lang="ru">
<head>	
	<meta charset="utf-8">
	<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
	<link rel="stylesheet" href="css/style.css">
	<script src="https://unpkg.com/just-validate@latest/dist/just-validate.production.min.js"></script>
	<script src="js/inputmask.min.js"></script>
	<title>Фотогалерея</title>
	
</head>

<body>






<div class="pop_up" id="reg">
	<div class="pop_up_container">
		<div class="pop_up_body">
			<div class="header1">
				<div class="container1">
					
						<a href="#" onclick="window.close()" class="pop_up_close">&#10006</a>
						<div class="nav">
							<a class="nav-item1" href="#" >Регистрация</a>
							<a class="nav-item2" href="#" >Авторизация</a>
						</div>
					
				</div>
			</div>

			<div class="registration__form">
				<form class="form" id="form" action="" method="post" autocomplete="off">
					<div class="container1">
						<div class="registration_name">
							<div class="ram1">
								<div class="qw">
									<input type="text" class="input" id="name" placeholder="Ваше имя˙" name="name" autocomplete="off">
									<p class="name">Ваше имя</p>
								</div>
							</div>
						</div>

						<div class="registration">

							<div class="ram23">
							<div class="str">
	
								<div class="ram2">
									<input type="email" class="input1" id="email" placeholder="Email" name="email" autocomplete="off">
									<p class="name2">Email</p>
								</div>
							</div>

							<div class="str">

								<div class="ram3">
									<input type="text" class="input1" id="phone" placeholder="Мобильный телефон" name="phone" autocomplete="off">
									<p class="name2">Мобильный телефон</p>
								</div>

							</div>
							</div>
							<div class="ram45">
							<div class="str">
								<div class="ram2">
									<input type="text" class="input2" placeholder="Пароль" autocomplete="off">
									<p class="name3">Пароль</p>
								</div>
							</div>
							<div class="str">
								<div class="ram3">
									<input type="text" class="input2" placeholder="Повторите пароль" autocomplete="off">
									<p class="name3">Повторите пароль</p>
								</div>
							</div>
							</div>
						</div>




                        			<div class="agree">
                        			    <label class="agree__checkbox__label">
                        			        <input type="checkbox" class="agree__checkbox">
                        			    </label>
                        			    <p class="agree__text">Согласен на обработку <a href=# class="agree__link">обработку персональных данных</a></p>
         			               </div>

						<div class="ram6">
							<div class="qw1">
								<button class="form_btn" type="submit">Зарегистрироваться</button>
							</div>
						</div>
						<div class="info">
							<div class="icon"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
							<circle cx="10" cy="10" r="7" stroke="#838383" stroke-width="2"/>
							<rect x="9" y="6" width="2" height="5" rx="1" fill="#838383"/>
							<rect x="9" y="12" width="2" height="2" rx="1" fill="#838383"/>
						</svg></div>
                             
                            			<p class="info__text">Все поля обязательны для заполнения</p>
                        			</div>
					</div>
				</form>
			</div>
		</div>
	</div>
</div>














<div class="pop_up1" id="inp">
	<div class="pop_up_container">
		<div class="pop_up_body">
			<div class="header1">
				<div class="container1">

						<a href="#" class="pop_up_close1">&#10006</a>

						<div class="nav1">
							<a class="nav-item3" href="#" >Регистрация</a>
							<a class="nav-item4" href="#" >Авторизация</a>
						</div>
				</div>
			</div>

			<div class="input__form">
				<form action="" method="post" autocomplete="off">
					<div class="container1">
						<div class="input_form">

							<div class="ram78">
							<div class="str">
	
								<div class="ram7">
									<input type="email" class="input1" placeholder="Email" autocomplete="off">
									<p class="name7">Email</p>
								</div>
								<div class="ram8">
									<input type="text" class="input2" placeholder="Пароль" autocomplete="off">
									<p class="name8">Пароль</p>
								</div>
							</div>
							</div>
						</div>

						<div class="ram9">
							<div class="qw2">
								<button class="form_btn" type="submit">Вход</button>
							</div>
						</div>
						<div class="info">
							<div class="icon"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
							<circle cx="10" cy="10" r="7" stroke="#838383" stroke-width="2"/>
							<rect x="9" y="6" width="2" height="5" rx="1" fill="#838383"/>
							<rect x="9" y="12" width="2" height="2" rx="1" fill="#838383"/>
						</svg></div>
                             
                            			<p class="info__text">Все поля обязательны для заполнения</p>
                        			</div>
					</div>
				</form>
			</div>
		</div>
	</div>
</div>













	<header class="header">
		<div class="container">
			<div class="header-line">
				<div class="header-logo">
					<img src="img/logo.png" alt="">
				</div>
				<div class="nav1">
					<a class="nav-item" href="#reg" >Регистрация</a>
					<a class="nav-item" href="#inp" >Вход</a>
				</div>
			</div>
			<div class="hr"></div>
		</div>
	</header>

	<div class="container">
		<div class="mes">
			<a class="text1">Čţëü, 2022</a>
			<div class="foto">
				<a href=# class="pl">+</a>
				<a href="#" class="bth">Добавить фото</a>
			</div>
		</div>
	</div>
	<div class="cards">
		<div class="container">
			<div class="cards-holder">
				<div class="card">
					<div class="text">
						<a class="card__title">July. Summer butterflies.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3.png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Summer butterflies.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (1).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Cape Point peninsula South Africa</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (2).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Antelope Canyon</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (3).png">
					</div>
				</div>
			
				<div class="card">
					<div class="text">
						<a class="card__title">Waves crashing in Brittany, France</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (4).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">After the rain.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (5).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Milkyway trail over death valley</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (6).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Nature</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (7).png">
					</div>
				</div>
			
				<div class="card">
					<div class="text">
						<a class="card__title">July. Summer butterflies.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (8).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">Wheat period.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (9).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">A baby jumping spider on yellow petals</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (10).png">
					</div>
				</div>
				<div class="card">
					<div class="text">
						<a class="card__title">July. Summer butterflies.</a>
						<p class="card__date">Добавлено 15 августа</p>
					</div>
					<div class="card-image">
						<img class="card-img" src="img/image 3 (11).png">
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="container">
	<div class="pokaz">
		<div class="qw">
			<img class="imgv" src="img/Vector 1.png">
			<a href=# class="show_more">Показать еще</a>
		</div>
	</div>
	<div class="hr1"></div>
		<div class="niz">
			<a href="#" class="nav-item5">info@gmail.com</a>

			<a href="#" class="nav-item6">Информация о разработчике</a>
		 
		</div>
	</div>
	</div>




	<script src="js/main.js"></script>








<?php

try{
$conn = new PDO("pgsql:host=localhost;dbname=fotobd", "postgres", "");
$request = $conn->prepare("SELECT * FROM test;");
$request->execute();
while($result = $request->fetch(PDO::FETCH_ASSOC)){
echo $result['test_column'];
echo "<br>";
}
$conn = null;
}
catch(Exception $e){
echo $e->getmessage();
}
echo 'Hello, world!';

?>







</body>
</html>
